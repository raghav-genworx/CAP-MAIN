...
export function assessmentToPayload(assessment: Assessment): AssessmentCreatePayload {
  return {
    ...
    hidden_feedback_mode: assessment.hidden_feedback_mode,
    max_hidden_checks: assessment.max_hidden_checks,
    hidden_check_cooldown_seconds: assessment.hidden_check_cooldown_seconds,
...
export function buildAssessmentQuestionAssignments(
  ...
  const ratios = questionIds.map((questionId) =>
    difficultyRatio(questionById.get(questionId)?.difficulty || "medium"),
  );
  const totalRatio = ratios.reduce((sum, ratio) => sum + ratio, 0) || 1;
  const rawMarks = ratios.map((ratio) => (ratio / totalRatio) * 100);
  const marks = rawMarks.map((item) => Math.floor(item));
  const remainder = 100 - marks.reduce((sum, mark) => sum + mark, 0);
  rawMarks
    .map((item, index) => ({ index, remainder: item - marks[index] }))
