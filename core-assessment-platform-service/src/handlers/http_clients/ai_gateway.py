...
class AIGatewayService:
    ...
    def __init__(self, settings: Settings, session: Session) -> None:
        ...
        self._current_target = (
            self._provider_targets[0] if self._provider_targets else None
        )

    @property
    def current_target_metadata(self) -> tuple[str, str]:
        """Return the provider and model currently selected by failover."""

        if self._current_target is None:
            return "unconfigured", "unconfigured"
        return self._current_target.provider, self._current_target.model
    ...
    def structured_completion(
        ...
        *,
        ...
        if not self._provider_targets:
            raise RuntimeError(
                "No AI provider is configured. Set GROQ_API_KEY_1..4, then "
            ...
            for target in self._provider_targets:
                ...
                try:
                    ...
                    if self._is_bad_request_error(exc):
                        disabled_target_labels.add(target.label)
                    LOGGER.warning(
                        "AI provider target failed for %s target=%s "
    ...
    def _is_target_exhaustion_error(cls, exc: Exception) -> bool:
        return (
            ...
            or cls._is_authentication_error(exc)
    ...
    def _supports_json_schema(target: _AIProviderTarget) -> bool:
        """Return whether the target supports Groq/OpenAI strict JSON schema."""

        if target.provider != "groq":
            return True
        return target.model.startswith("openai/gpt-oss-")
    ...
    def _strict_json_schema(cls, value: Any) -> Any:
        ...
        normalized = {key: cls._strict_json_schema(item) for key, item in value.items()}
