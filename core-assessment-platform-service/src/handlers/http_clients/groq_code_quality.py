...
class GroqCodeQualityEvaluator:
    ...
    def evaluate(self, *, language: str, source_code: str) -> AICodeQualitySignal:
        ...
        try:
            # Each tenacity attempt bridges into the async transport; the retry
            # policy itself stays synchronous so its behaviour is unchanged.
            return retrying(lambda p: run_async(self._request_review_async, p), prompt)
    ...
    async def _request_review_async(self, prompt: str) -> AICodeQualitySignal:
