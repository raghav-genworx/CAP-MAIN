...
class HttpContractTests(unittest.TestCase):
    ...
    def test_request_id_is_generated_and_propagated(self) -> None:
        ...
        propagated = client.get(
            "/items/2",
            headers={"X-Request-ID": "core-contract-42"},
        )

        self.assertRegex(generated.headers["x-request-id"], r"^[a-f0-9]{32}$")
        self.assertEqual(propagated.headers["x-request-id"], "core-contract-42")
    ...
    def test_validation_error_contains_trace_id(self) -> None:
        ...
        response = client.get(
            ...
            headers={"X-Request-ID": "core-validation-42"},
        )

        self.assertEqual(response.status_code, 422)
        self.assertEqual(response.json()["trace_id"], "core-validation-42")
